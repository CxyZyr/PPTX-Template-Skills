...
def _fit_title(shape, text, *, font, color, min_left=None, max_right=None,
    ...
    fit_title(
        shape, text,
        base_size_pt=base, min_size_pt=max(12, int(base * 0.6)),
...
def _title_allows_break(shape, slide_width: int | None) -> bool:
...
def _fit_body(shape, paragraphs, *, font, color, slide_width=None,
              visual_box=None, max_abs_right=None):
    base = round(_current_pt(shape, 16))
    max_right = (slide_width - inch_to_emu(0.35)) if slide_width else None
    ...
    resize_width = True
    if visual_box and max_abs_right is not None:
...
def _layout_limits(own_box, layout_boxes: list[dict], *, own_path=None,
    ...
    if container_box:
        ...
        if top >= c_top - gutter and left < c_left + c_width and left + width > c_left:
            ...
            if candidate > top + inch_to_emu(0.18):
                limits["max_abs_bottom"] = min(limits.get("max_abs_bottom", candidate), candidate)
    ...
    for item in layout_boxes:
        ...
        if (
            v_ov >= min(height, o_height) * 0.25
            and o_left > left + width * 0.45
        ):
        ...
        if (
            ...
            if candidate > top + inch_to_emu(0.18):
                limits["max_abs_bottom"] = min(
                    limits.get("max_abs_bottom", candidate),
                    candidate,
                )
    return limits


def _contents_label_row_limits(slide, label_slots: list[dict],
                               labels_content: list, *, page_role=None) -> dict[tuple, int]:
    """Constrain parallel contents labels to their template column width."""
    if page_role != "contents" or len(labels_content) < 3:
        return {}
...
def _fill_cards(slide, resolved_cards, cards_content, *, asset_dir, brand, warnings,
    ...
    for i in range(filled):
        ...
        if slot["body"] and not shared_textbox and content.get("body"):
...
def _fill_body_slots(resolved_body, paragraphs, *, font, warnings, slots=None,
    ...
    if page_role == "contents" and count >= 3:
        _normalize_text_font_size(shapes[:count])
    return count
...
def apply_page(slide, fill_plan: dict, page: dict, *, asset_dir,
    ...
    if page.get("title") and res["title"] is not None:
        ...
        if _matches_first_template_paragraph(res["title"], title_text):
            ...
            _fit_title(res["title"], page["title"], font=title_font_for_slot, color=title_color,
                       ...
                       allow_break=_title_allows_break(res["title"], slide_width),
                       respect_bounds=bool(title_limits),
                       bold=title_bold)
    ...
    # footer
    if page.get("footer") and res["footer"] is not None:
        ...
        if "footer" in page and page.get("footer") == "":
            _clear_text(res["footer"])
        else:
            _clear_if_unfilled(res["footer"], fill_plan.get("footer"))

    # labels - treat like body slots but simpler (no autofit)
    label_slots = fill_plan.get("labels", [])
    if "labels" in page:
        labels_content = _as_list(page["labels"])
        label_row_limits = _contents_label_row_limits(
            slide,
            label_slots,
            labels_content,
        ...
        for i, shape in enumerate(res["labels"]):
            ...
            if shape is not None and i < len(labels_content):
                ...
                _clear_if_unfilled(shape, slot)
        if n > 0:
            if page.get("role") == "contents" and n >= 3:
                _normalize_text_font_size(res["labels"][:n])
    ...
    # cards
    if page.get("cards"):
        n = _fill_cards(slide, res["cards"], page["cards"],
